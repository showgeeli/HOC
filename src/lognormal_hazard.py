...
cv = 1.3        # target cv
T = 50
sigma = np.sqrt(np.log(cv**2 +1))           # analytical expression of sigma
mu = abs(-0.5 * np.log(rate**2 * (cv**2 +1)))    # analytical expression of mu
print(sigma)
print(mu)    



# define hazard function of lognormal distribution
def hazard(t):
...
def thinning(S, hazard, R_m):
    n = len(S)
    U = np.random.uniform(0,R_m, n) # to store all the random U values
    t_latest = S[0] 
    new_S = [t_latest]
    for i in range(1,n):
...
S = pp.generate_parallelST([R_m], T)[0]
new_S = thinning(S, hazard, R_m)
